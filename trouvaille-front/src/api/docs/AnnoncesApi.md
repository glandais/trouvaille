...
const { status, data } = await apiInstance.createAnnonce(
    annonceCreate
);
```
...
const { status, data } = await apiInstance.deleteAnnonce(
    id
);
```
...
const { status, data } = await apiInstance.getAnnonce(
    id
);
```
...
const { status, data } = await apiInstance.putAnnonce(
    id,
    annonceUpdate
);
```
